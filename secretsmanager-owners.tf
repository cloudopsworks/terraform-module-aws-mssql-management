...
resource "aws_secretsmanager_secret" "owner" {
  ...
  description = "RDS Owner credentials - ${local.owner_list[each.key]} - ${local.psql.engine} - ${local.psql.server_name} - ${mssql_database.this[each.key].name}"
  kms_key_id  = var.secrets_kms_key_id
  tags = merge(local.all_tags, {
    "rds-username"        = local.owner_list[each.key]
    "rds-datatabase-name" = mssql_database.this[each.key].name
...
resource "aws_secretsmanager_secret_version" "owner" {
  ...
  secret_string = jsonencode({
    ...
    dbname = mssql_database.this[each.key].name
