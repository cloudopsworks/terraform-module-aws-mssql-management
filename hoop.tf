...
locals {
  hoop_enabled       = try(var.hoop.enabled, false) && strcontains(local.psql.engine, "sqlserver")
  ...
  value = local.hoop_enabled ? merge(
    {
      ...
      "${local.psql.server_name}-${try(db.create, true) ? mssql_database.this[key].name : data.mssql_database.this[key].name}-ow" => {
        name           = "${local.psql.server_name}-${try(db.create, true) ? mssql_database.this[key].name : data.mssql_database.this[key].name}-ow"
    ...
    },
    {
      ...
      "${local.psql.server_name}-${try(role_user.db_ref, "") != "" ? mssql_database.this[role_user.db_ref].name : role_user.database_name}-${role_user.name}" => {
        name           = "${local.psql.server_name}-${try(role_user.db_ref, "") != "" ? mssql_database.this[role_user.db_ref].name : role_user.database_name}-${role_user.name}"
        agent_id       = var.hoop.agent_id
        type           = "database"
        subtype        = "sqlserver"
        tags           = try(var.hoop.tags, {})
